...
try {
    ...
    $BINARY_PATH = Join-Path $TEMP_DIR "agentc.exe"
    if (-not (Test-Path $BINARY_PATH)) {
        Write-Host "❌ 解压后未找到 agentc.exe 二进制文件" -ForegroundColor Red
        exit 1
    }

    # 安装到用户目录
    $INSTALL_DIR = Join-Path $env:USERPROFILE "bin"
    $INSTALL_PATH = Join-Path $INSTALL_DIR "agentc.exe"
    ...
    Write-Host "✅ agentc 已安装到 $INSTALL_PATH" -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "安装完成！重启终端后运行 'agentc --help' 查看使用说明" -ForegroundColor Green
}
finally {
    # 清理临时文件
