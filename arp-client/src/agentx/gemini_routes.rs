...
pub fn register_gemini_project_routes(router_builder: &mut RouterBuilder) {
    router_builder.get("/api/gemini/projects", |ctx| async move {
        let mut stream = ctx.stream;
        match gemini::list_projects().await {
            Ok(projects) => {
                let body = json!({
                    "type": "projects",
                    "projects": projects
    ...
    router_builder.get(
        "/api/gemini/projects/working-directories",
        |ctx| async move {
            let mut stream = ctx.stream;
            match gemini::get_working_directories().await {
                Ok(directories) => {
                    let body = json!({
...
pub fn register_gemini_session_routes(router_builder: &mut RouterBuilder) {
    router_builder.get("/api/gemini/sessions", |ctx| async move {
        let limit = ctx
            .request
            .query_param("limit")
        ...
        let mut stream = ctx.stream;
        match gemini::get_all_sessions(limit, offset, project_path).await {
            Ok(sessions) => {
                let body = json!({
                    "type": "sessions",
                    "sessions": sessions
    ...
    router_builder.get("/api/gemini/sessions/{session_id}", |ctx| async move {
        ...
        let mut stream = ctx.stream;
        match gemini::load_session_by_id(session_id.clone()).await {
            Ok(messages) => {
                let body = json!({
    ...
    router_builder.delete("/api/gemini/sessions/{session_id}", |ctx| async move {
        let session_id = match ctx.path_params.get("session_id") {
            Some(v) if !v.is_empty() => v.clone(),
            _ => {
                let mut stream = ctx.stream;
        ...
        let mut stream = ctx.stream;
        match gemini::delete_session_by_id(session_id.clone()).await {
            Ok(_) => {
                let body = json!({
